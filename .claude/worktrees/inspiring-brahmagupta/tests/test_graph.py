...
async def test_pendingClaimEndsAfterIntake():
    ...
    with patch("agentic_claims.core.graph.intakeNode", mockIntakeNode):
        ...
        assert "Compliance Agent" not in allContent, "Compliance should NOT run"
...
async def test_submittedClaimRoutesToComplianceAndFraud():
    ...
    with patch("agentic_claims.core.graph.intakeNode", mockIntakeNode):
        ...
        # jamesoon: Updated assertions for real agent output (no longer stubs)
        assert "Compliance check" in allContent or "Compliance Agent" in allContent, "Compliance should run"
        assert "Fraud check" in allContent or "Fraud Agent" in allContent, "Fraud should run"
        assert "Advisor decision" in allContent or "Advisor Agent" in allContent, "Advisor should run"
