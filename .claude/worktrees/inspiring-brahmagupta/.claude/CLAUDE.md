...
Running log of things not to try again. Updated when Claude makes a mistake during development.

<!-- Add entries below in reverse chronological order -->
<!-- Format: - **YYYY-MM-DD**: Description of what went wrong and what to do instead -->

- **2026-04-04**: RDS engine version 16.4 doesn't exist in ap-southeast-1. Always query available versions first with `aws rds describe-db-engine-versions` before hardcoding. Used 16.6 instead.
- **2026-04-04**: EC2 user-data script tried `git clone` on a private repo — failed with "could not read Username". Private repos need deploy keys, tokens, or SCP the code directly. Solution: tar + SCP the source code, build on EC2.
- **2026-04-04**: Docker Compose v5+ requires `docker-buildx` plugin which isn't included in Amazon Linux 2023 `docker` package. Must install buildx separately: `curl -SL https://github.com/docker/buildx/releases/download/v0.22.0/buildx-v0.22.0.linux-amd64 -o /usr/local/lib/docker/cli-plugins/docker-buildx`.
- **2026-04-04**: EC2 Instance Connect SSH key expires after 60 seconds. Must call `aws ec2-instance-connect send-ssh-public-key` immediately before each SSH/SCP command. Cannot reuse a pushed key across multiple commands with delays.
- **2026-04-04**: macOS tar adds extended attributes (SCHILY.fflags, LIBARCHIVE.xattr) that produce noisy warnings on Linux. Harmless but clutters logs. Use `COPYFILE_DISABLE=1 tar czf` on macOS to suppress.
- **2026-04-04**: Never put API keys directly in files that will be committed (setup-ec2.sh). Use a separate `.secrets` file that's gitignored, and have deploy.sh source it and inject via sed placeholders.
- **2026-04-04**: Build Docker images locally (30s) not on t3.micro (10+ min). Workflow: `docker build --platform linux/amd64` → `docker save | gzip` → S3 upload → EC2 `docker load`. Cross-platform flag required (Mac ARM → EC2 x86).
- **2026-04-04**: Docker Desktop on Mac is at `/Applications/Docker.app/Contents/Resources/bin/docker`, NOT in PATH. Need full path or PATH export. Also needed for docker-credential-desktop to resolve.
- **2026-04-04**: System Python 3.9 on macOS can't create venvs. Must use Homebrew Python 3.11: `brew install python@3.11`. Poetry: `curl -sSL https://install.python-poetry.org | python3.11 -`. Add `$HOME/.local/bin` to PATH.
- **2026-04-04**: `getSettings()` throws in test environment (no .env.local). Pattern: wrap in try/except with fallback URL before MCP calls. Otherwise mock is never reached because exception happens at `getSettings()` not at `mcpCallTool()`.
- **2026-04-04**: Qdrant Docker image has no `curl` or `wget`. Healthcheck must use `bash -c 'echo > /dev/tcp/localhost/6333'` instead.
- **2026-04-04**: t3.micro (1GB RAM) cannot run 6 containers + docker load simultaneously. Use t3.small (2GB) minimum. Instance becomes SSH-unresponsive when out of memory.
- **2026-04-04**: .env.local was missing 15 fields that Settings requires: fallback models, max_tokens, temperature, MCP URLs, image thresholds, log/seq settings. Always check `src/agentic_claims/core/config.py` for the full field list before deploying.
