...
def _uploadsDir() -> Path:
    """Resolve writable uploads directory, creating it if needed.

    Prefers /data/uploads (Docker writable volume) over static/uploads
    which may be mounted read-only.
    """
    dockerUploads = Path("/data/uploads")
    if dockerUploads.parent.exists():
        dockerUploads.mkdir(parents=True, exist_ok=True)
