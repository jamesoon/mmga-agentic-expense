...
async def runGraph(graph, graphInput: dict, request: Request, templates: Jinja2Templates):
    ...
    modelShort = settings.openrouter_model_llm.split("/")[-1] if "/" in settings.openrouter_model_llm else settings.openrouter_model_llm
    yield ServerSentEvent(raw_data=f"Calling {modelShort}...", event=SseEvent.STEP_NAME)
        ...
        async for event in graph.astream_events(invokeInput, config=config, version="v2"):
            ...
            if eventKind == "on_chat_model_stream":
                ...
                if chunk and hasattr(chunk, "content") and chunk.content:
                    if not receivedFirstToken:
                        receivedFirstToken = True
                        elapsed = _formatElapsed(time.time() - turnStart)
                        yield ServerSentEvent(raw_data=f"{modelShort} responding ({elapsed})...", event=SseEvent.STEP_NAME)
